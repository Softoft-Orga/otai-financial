...
DEFAULT_ASSUMPTIONS = Assumptions(
    ...
    base_organic_users_per_month=10.0,  # Very low baseline - reflects minimal organic traction without marketing
    ...
    cpc_base=1.5,  # Base CPC at low spend levels (best case scenario)
    cpc_k=0.25,  # CPC scaling factor - controls how quickly CPC increases with spend
    ...
    seo_eff_users_per_eur=0.25,  # Users generated per EUR spent on SEO content/links
    ...
    domain_rating_growth_k=0.07,  # Growth coefficient for DR improvement from SEO spend
    domain_rating_growth_ref_spend=1500.0,  # Reference monthly SEO spend for DR growth calculations
    ...
    conv_web_to_lead=0.02,  # 2% of website visitors become leads (industry average: 2-5%)
    conv_website_lead_to_free=0.15,
    conv_website_lead_to_pro=0.01,
    conv_website_lead_to_ent=0.001,
    conv_outreach_lead_to_free=0.05,
    conv_outreach_lead_to_pro=0.03,
    conv_outreach_lead_to_ent=0.005,
    conv_free_to_pro=0.05,  # 5% of free users upgrade to paid (industry average: 2-10%)
    conv_pro_to_ent=0.01,  # 1% of pro users upgrade to enterprise (typical for high-touch sales)
    ...
    pro_price_base=5000.0,  # Base price for pro plan (5K EUR/year = ~417 EUR/month)
    ent_price_base=20000.0,  # Base price for enterprise plan (20K EUR/year = ~1667 EUR/month)
    pro_price_k=0.05,  # Price scaling factor for pro plan based on product value
    ent_price_k=0.03,  # Price scaling factor for enterprise plan based on product value
    ...
    support_cost_per_ent=300.0,  # 300 EUR/month per enterprise customer (3600 EUR/year - high-touch support)
    # Optional support subscription (premium support add-on)
    support_subscription_fee_pct_pro=0.10,  # 10% of base price for pro support subscription (500 EUR/year)
    support_subscription_fee_pct_ent=0.10,  # 10% of base price for enterprise support subscription (2000 EUR/year)
    ...
    operating_baseline=3000.0,  # Fixed baseline costs (tools, software, basic infrastructure)
    ...
    qualified_pool_total=5_000.0,  # Total addressable prospects in the market (finite pool)
    scraping_efficiency_k=0.8,  # Efficiency factor for scraping (diminishing returns with spend)
    scraping_ref_spend=800.0,  # Reference monthly spend for scraping calculations
    ...
    pv_ref=100.0,  # Reference product value for calculations
    pv_decay_shape=0.06,  # Natural monthly decay without dev spend (6% at pv_ref)
    pv_growth_scale=0.18,  # Growth efficiency from dev spend (18% at pv_ref)
    product_value_divisor=1000.0,  # Divisor for normalizing product value in formulas
    # Product value impact coefficients (how strongly product value affects various metrics)
    k_pv_web_to_lead=0.30,  # 30% max improvement in web-to-lead conversion from product value
    k_pv_lead_to_free=0.15,  # 15% max improvement in lead-to-free conversion
    k_pv_free_to_pro=0.25,  # 25% max improvement in free-to-pro conversion
    k_pv_pro_to_ent=0.20,  # 20% max improvement in pro-to-enterprise conversion
    k_pv_churn_pro=0.20,  # 20% max reduction in pro churn from product value
    k_pv_churn_free=0.10,  # 10% max reduction in free churn (free users less sensitive)
    k_pv_churn_ent=0.15,  # 15% max reduction in enterprise churn (already low, less room for improvement)
)
