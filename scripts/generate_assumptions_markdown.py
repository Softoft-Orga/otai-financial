...
""")
    ...
    lines.append("### Outreach / Lead Scraping")
    lines.append("")
    lines.append("Direct outreach finds leads from a finite qualified pool that depletes over time.")
    lines.append("")
    lines.append("```")
    lines.append("raw_leads  = outreach_budget × (outreach_leads_per_1000_eur / 1000)")
    lines.append("contacted  = remaining_pool × (1 − exp(−raw_leads / remaining_pool))")
    lines.append("```")
    lines.append("")
    lines.append("- At **low spend**: behaves linearly — you get ~`outreach_leads_per_1000_eur` leads per €1,000.")
    lines.append("- At **high spend**: saturates toward the remaining pool (can never exceed it).")
    lines.append("- The pool **shrinks each month** as leads are contacted, creating natural long-term diminishing returns.")
    lines.append("")
    lines.append(f"- **Qualified pool**: {DEFAULT_ASSUMPTIONS.qualified_pool_total:,.0f}")
    lines.append(f"- **Leads per €1,000**: {DEFAULT_ASSUMPTIONS.outreach_leads_per_1000_eur:,.0f}")
    lines.append(f"- **Cost per lead**: {DEFAULT_ASSUMPTIONS.cost_per_direct_lead:.2f} EUR")
    lines.append(f"- **Cost per demo**: {DEFAULT_ASSUMPTIONS.cost_per_direct_demo:.2f} EUR")
    lines.append("")
    
    lines.append("### Debt interest rate")
    lines.append("")
    lines.append("The interest rate scales smoothly between a base and maximum rate based on how")
    lines.append("much debt the company carries relative to its revenue.")
    lines.append("")
    lines.append("```")
    lines.append("risk_ratio = total_debt / (total_debt + annual_revenue)")
    lines.append("rate = base_rate + (max_rate − base_rate) × risk_ratio")
    lines.append("```")
    lines.append("")
    lines.append("**How to read this:**")
    lines.append("")
    lines.append("- **No debt** → 0% interest (nothing to pay).")
    lines.append("- **Low debt vs revenue** → near the base rate.")
    lines.append("- **Debt = revenue** → midpoint between base and max.")
    lines.append("- **Debt >> revenue** → approaches max rate.")
    lines.append("- **No revenue** → max rate (highest risk).")
    lines.append("")
    lines.append(f"- **Base rate**: {DEFAULT_ASSUMPTIONS.debt_interest_rate_annual:.2%}")
    lines.append(f"- **Max rate**: {DEFAULT_ASSUMPTIONS.debt_interest_rate_max_annual:.2%}")
    lines.append("")
...
def build_markdown() -> str:
    ...
    lines.append("## Outreach / Lead Scraping")
    lines.append("")
    lines.append("Direct outreach targets a finite pool of qualified prospects. Each month, your outreach budget")
    lines.append("finds leads from the **remaining** pool. The number of leads found follows a simple rule:")
    lines.append("")
    lines.append("```")
    lines.append("raw_leads  = outreach_budget × (outreach_leads_per_1000_eur / 1000)")
    lines.append("contacted  = remaining_pool × (1 − exp(−raw_leads / remaining_pool))")
    ...
    lines.append("**How to read this:**")
    lines.append("")
    lines.append("- At **low spend**, the formula is nearly linear: you get roughly `outreach_leads_per_1000_eur` leads per €1,000 spent.")
    lines.append("- At **high spend**, diminishing returns kick in — you can never contact more than the remaining pool in one month.")
    lines.append("- The pool **depletes over time** as leads are contacted, providing natural long-term diminishing returns.")
    lines.append("")
    lines.append("Each contacted lead incurs an additional `cost_per_direct_lead` (e.g. email enrichment, sending), ")
    lines.append("and each demo appointment costs `cost_per_direct_demo`.")
    lines.append("")
    lines.append("| Parameter | Value | Meaning |")
    lines.append("| --- | --- | --- |")
    lines.append(f"| Qualified pool total | {_format_number(a.qualified_pool_total)} | Total addressable prospects |")
    lines.append(f"| Leads per €1,000 | {_format_number(a.outreach_leads_per_1000_eur)} | Leads found per €1k at low volumes |")
    lines.append(f"| Cost per direct lead | {_format_currency(a.cost_per_direct_lead)} | Cost to contact one found lead |")
    lines.append(f"| Cost per direct demo | {_format_currency(a.cost_per_direct_demo)} | Cost to run one demo appointment |")
    lines.append("")

    lines.append("## Debt & Credit")
    lines.append("")
    lines.append("The simulation includes debt financing with variable interest rates. The rate scales")
    lines.append("smoothly between a base and maximum based on the debt-to-revenue ratio:")
    lines.append("")
    lines.append("```")
    lines.append("risk_ratio = total_debt / (total_debt + annual_revenue)")
    lines.append("rate = base_rate + (max_rate − base_rate) × risk_ratio")
    lines.append("```")
    lines.append("")
    lines.append("- **No debt** → 0% interest. **Low debt** → near base rate. **Debt >> revenue** → max rate.")
    lines.append("")
    lines.append("The credit draw factor determines how much credit is drawn when cash is low,")
    lines.append("while the debt repay factor controls repayment speed when cash is available.")
    lines.append("")
    lines.append("| Parameter | Value | Meaning |")
    lines.append("| --- | --- | --- |")
    lines.append(f"| Base interest rate | {_format_percent(a.debt_interest_rate_annual)} | Rate when debt is low vs revenue |")
    lines.append(f"| Max interest rate | {_format_percent(a.debt_interest_rate_max_annual)} | Rate when debt >> revenue |")
    lines.append(f"| Credit draw factor | {_format_number(a.credit_draw_factor)} | Multiplier on negative cashflow for credit |")
    lines.append(f"| Debt repay factor | {_format_number(a.debt_repay_factor)} | Multiplier on positive cashflow for repayment |")
    lines.append("")
